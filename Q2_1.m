...
% hyteresis(SP,T_Low,T_High,A);

end

% %Hysteresis Thresholding
% function H=hyteresis(bh,t_w,t_h,a)
% 
% t_w = t_w * max(max(bh));
% t_h = t_h * max(max(bh));
% pan=size(a,1);
...
%         elseif ( bh(i+1,j)>t_h || bh(i-1,j)>t_h || bh(i,j+1)>t_h || bh(i,j-1)>t_h || bh(i-1, j-1)>t_h || bh(i-1, j+1)>t_h || bh(i+1, j+1)>t_h || bh(i+1, j-1)>t_h)
%             T_res(i,j) = 1;
%         end;
%     end;
% end;
